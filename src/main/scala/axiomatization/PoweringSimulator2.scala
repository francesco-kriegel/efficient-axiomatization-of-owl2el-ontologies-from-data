...
class PoweringClosureOperator3(graph: BitGraph[OWLClass, OWLObjectProperty],
  ...
  extends PoweringSimulator2(graph, graph, knownSimulation, maxConjunctionSize, throwExceptionWhenSomeConjunctionIsTooLarge, maxRoleDepth) {
...
class PoweringSimulator2(val source: BitGraph[OWLClass, OWLObjectProperty],
  ...
  val simulation = if knownSimulation.isDefined then knownSimulation.get else GraphSimulator.computeMaximalSimulation3(source, target)
  ...
  def apply(xs: collection.BitSet): collection.BitSet = {
    ...
    if (exception.isDefined) {
      ...
      val poweringSimulation = GraphSimulator.computeMaximalSimulation3(powering, target)
