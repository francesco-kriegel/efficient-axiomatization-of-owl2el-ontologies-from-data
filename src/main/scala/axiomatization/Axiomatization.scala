...
object Axiomatization2 {
  ...
  def run(ontologyFile: File,
    ...
    val ((reduction, _, _, _), measurement_ComputationTime_FirstReduction) =
      ...
      else measureExecutionTime { Interpretation.reductionOf2(graph) }
    ...
    var (reducedCanonicalModel, _rcmRepresentedBy, _rcmRepresentativeOf, simulationOnRCM) = Interpretation.reductionOf2(elk.canonicalModel)
    ...
    val (closureToGeneratorMap, measurement_ComputationTime_Closures) = measureExecutionTime {
      if (maxRoleDepth.isDefined && (maxRoleDepth.get equals 0))
        ...
        if (withDisjointnessAxioms)
          FCbOPar2.computeAllClosures(n, PoweringClosureOperator3(reducedCanonicalModel, Some(simulationOnRCM), maxConjunctionSize, false, maxRoleDepth.map(_ - 1)))
            .subtractOne(reducedCanonicalModel.nodes())
        else
          FCbOPar2.computeAllClosures(n, PoweringClosureOperator3(reducedCanonicalModel, Some(simulationOnRCM), maxConjunctionSize, false, maxRoleDepth.map(_ - 1)), isOccupiedAttribute)
            .subtractOne(reducedCanonicalModel.nodes())
      }
    }
    //    valueLogger.close()
    ...
    val (reducedCanonicalModel2, _rcmRepresentedBy2, _rcmRepresentativeOf2, _) = Interpretation.reductionOf2(elk.canonicalModel)
    ...
    val poweringSimulator = PoweringSimulator2(reducedCanonicalModel, tboxSaturation, Some(simulationOnRCM), None, false, maxRoleDepth.map(_ - 1))
...
//    val clop = PoweringClosureOperator_Incremental(tboxSaturation)
//    val clop = PoweringClosureOperator2_CachedValues(tboxSaturation)
    val poweringSimulator = PoweringSimulator(reducedCanonicalModel, Some(simulationOnRCM), maxRoleDepth.map(_ - 1), tboxSaturation)
